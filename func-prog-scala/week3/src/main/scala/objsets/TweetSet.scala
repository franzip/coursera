...
abstract class TweetSet {
  ...
  def retweetAcc(acc: Tweet): Tweet
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = retweetAcc(elem)
  
  def retweetAcc(acc: Tweet): Tweet = {
    left.retweetAcc(right.retweetAcc(if (elem.retweets > acc.retweets) elem else acc))
  }
