...
def equal(A, B):
    ...
    """
    assert A.D == B.D
    return not False in [getitem(A, (x, y)) == getitem(B, (x, y)) for x in A.D[0] for y in A.D[1]]
...
def add(A, B):
    ...
    """
    assert A.D == B.D
    return Mat(A.D, {(x, y):(getitem(A, (x, y)) + getitem(B, (x, y)))
                     for x in A.D[0] for y in A.D[1]})
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat(M.D, {key: getitem(M, key) * x for key in M.f.keys()})
...
def vector_matrix_mul(v, M):
    ...
    """
    assert M.D[0] == v.D
    return Vec(M.D[1], {col: sum([v[row] * getitem(M, (row, col)) for row in M.D[0]])
                        for col in M.D[1]})
...
def matrix_vector_mul(M, v):
    ...
    """
    assert M.D[1] == v.D
    return Vec(M.D[0], {row: sum([v[col] * getitem(M, (row, col)) for col in M.D[1]])
                        for row in M.D[0]})
...
def matrix_matrix_mul(A, B):
    ...
    for x in A.D[1]:
        for y in A.D[0]:
            for z in B.D[1]:
                result[(y,z)] += getitem(A, (y, x)) * getitem(B, (x, z))
    return result
