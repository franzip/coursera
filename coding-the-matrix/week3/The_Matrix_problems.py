...
def lin_comb_mat_vec_mult(M, v):
    ...
    assert(M.D[1] == v.D)
    cols = mat2coldict(M)
    scaled_cols = [v[k] * cols[k] for k in v.D]
    return Vec(M.D[0], {rowcol[0]: sum([vec[rowcol[0]] for vec in scaled_cols])
                        for rowcol in zip(M.D[0], M.D[1])})
...
def lin_comb_vec_mat_mult(v, M):
    ...
    assert(v.D == M.D[0])
    rows = mat2rowdict(M)
    scaled_rows = [v[k] * rows[k] for k in v.D]
    return Vec(M.D[1], {rowcol[1]: sum([vec[rowcol[1]] for vec in scaled_rows])
               for rowcol in zip(M.D[0], M.D[1])})
...
def dot_product_mat_vec_mult(M, v):
    ...
    assert(M.D[1] == v.D)
    rows = mat2rowdict(M)
    return Vec(M.D[0], {row: rows[row] * v for row in M.D[0]})
...
def dot_product_vec_mat_mult(v, M):
    ...
    assert(v.D == M.D[0])
    cols = mat2coldict(M)
    return Vec(M.D[1], {col: cols[col] * v for col in M.D[1]})
...
def Mv_mat_mat_mult(A, B):
    ...
    AB = {col: A * col_dict[col] for col in col_dict}
...
def vM_mat_mat_mult(A, B):
    ...
    AB = {row: row_dict[row] * B for row in row_dict}
