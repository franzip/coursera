...
gf2_lc_rep_1 = [one,0,one,one,one,0,0,0]
gf2_lc_rep_2 = [one,one,one, 0, one, one, one, one]
gf2_lc_rep_3 = [one, one, 0, 0, one, 0, 0, 0]
...
indep_vec_4 = Vec({0,1,2}, {0:1, 1:2, 2:3})
...
def exchange(S, A, z):
    ...
    for x in range(len(result)):            
         if is_superfluous(result, x) and result[x] not in A and result[x] != z:          
              return result[x]
