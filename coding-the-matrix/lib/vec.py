...
def equal(u,v):
    ...
    """
    assert u.D == v.D
    # Kinda hacky solution exploiting python overloading
    return sum([getitem(v,x) == getitem(u,x) for x in v.D]) == len(v.D)
...
def add(u,v):
    ...
    """
    assert u.D == v.D
    return Vec(u.D, {x: getitem(v,x) + getitem(u,x) for x in u.D})
...
def dot(u,v):
    ...
    """
    assert u.D == v.D
    return sum([getitem(v,x) * getitem(u,x) for x in u.D])
...
def neg(v):
    ...
    True
    """
    return Vec(v.D, {x: -getitem(v, x) for x in v.D})
