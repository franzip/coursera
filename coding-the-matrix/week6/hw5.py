...
def is_invertible(M): 
    ...
    True
    '''
    squared = len(M.D[0]) == len(M.D[1])
    final = []
    for x in mat2coldict(M):
        final.append(mat2coldict(M)[x])
    return squared and is_independent(final)
