...
from mat import Mat
from vec import Vec
from The_Basis_problems import is_independent, is_superfluous
...
def my_rank(L):
    ...
    Example:
        ...
        2
    '''
    L_s = [vec for vec in L if not vec.is_almost_zero()]
    return len(subset_basis(L_s))
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    '''
    UV = U_basis + V_basis
    x = solve(coldict2mat(UV), w)
    uc_D = set(range(len(U_basis)))
    uc = Vec(uc_D, {i: x[i] for i in uc_D})
    vc_D = set(range(len(V_basis)))
    vc = Vec(vc_D, {i: x[i + len(U_basis)] for i in vc_D})
    return (coldict2mat(U_basis) * uc, coldict2mat(V_basis) * vc)
